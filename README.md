...
- [完整命令清单（60+）](#完整命令清单60)
- [Writing Subskills（21 个写作子技能）](#writing-subskills21-个写作子技能)
...
│   ├── style-polish/              agent-style 21 规则润色（去 AI 味）
...
> `gen-outline` 是骨架生成器（非关卡），归在 `tools/`，用 `/ccg-paper:gen-outline` 调用。
...
| `/ccg-paper:style-polish` | agent-style 21 规则润色，去除 AI 腔 | "AI 味太重" / "润色" |
